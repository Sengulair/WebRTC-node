...
module.exports = function(modem) {
  ...
  modem.addListener({
    ...
    process: (newpart) => {
      const rest = newpart.substring(7);
      const coma = rest.indexOf(',');
      const splitted_newpart = [];
      if (coma < 0) {
        splitted_newpart.push(rest);
      } else {
        splitted_newpart.push(rest.substring(0,coma));
        splitted_newpart.push(rest.substring(coma+1));
      }
